...
function AnketBuglukh() {
  ...
  const onFinish = (values) => {
    // Convert array values to strings for backend compatibility
    const processedValues = {
      ...values,
      asuultuud: values.asuultuud.map(({ asuult, khariult }) => ({
        asuult,
        khariult: Array.isArray(khariult) ? khariult.join(", ") : khariult,
      })),
    };

    // Check all text fields for bad words
    const allText = [
      processedValues.tailbar || "",
      ...processedValues.asuultuud.map(({ khariult }) => khariult || ""),
    ].join(" ");

    const hasBadWords = detectBadWords(allText);

    // Calculate score and check thresholds
    const { score, message } = calculateScore(processedValues);
    const hasLowScore =
    ...
    const khariult = {
      ...
      utas: processedValues.utas,
      ognoo: new Date(),
      khariultuud: processedValues.asuultuud.map(({ asuult, khariult, turul }, index) => {
      ...
      tailbar: processedValues.tailbar,
